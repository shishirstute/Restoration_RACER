...
class PVLoadForecaster:
    ...
    def __init__(
            ...
            rated_p: float,
            phase_index: int = 0,
            pv_file: str = "estimated_power_generation_testing_1_day_15_min.csv",
            irr_file: str = "forecasted_1_day_15m_interval_irradiance.csv",
            irr_cutoff: float = 200.0,
    ):
        self.rated_p = rated_p  # ← NEW / restored
        self.phase_index = phase_index
        self.pv_file = pv_file
        self.irr_file = irr_file
        self.irr_cutoff = irr_cutoff
    ...
    def _generate_24h_pv_forecast(self):
        pv_vals = pd.read_csv(self.pv_file)["estimated_power_generation"].to_numpy()
        irr_vals = pd.read_csv(self.irr_file, header=None).to_numpy().flatten()

        # zero PV when sun is too low
        pv_vals = np.where(irr_vals <= self.irr_cutoff, 0.0, pv_vals)
        # make sure we never exceed the inverter / array size
        pv_vals = np.minimum(pv_vals, self.rated_p)

        self.pv_forecast = pv_vals.tolist()

        # zero PV when irradiance is below the threshold
        pv_vals = np.where(irr_vals <= self.irr_cutoff, 0.0, pv_vals)
        self.pv_forecast = pv_vals.tolist()
    # ---------- main module ---------------------------------------------------------
