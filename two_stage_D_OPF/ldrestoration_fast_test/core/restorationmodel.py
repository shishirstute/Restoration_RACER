...
from ldrestoration_fast_test.utils.decors import timethis
from ldrestoration_fast_test.core.dataloader import DataLoader
from ldrestoration_fast_test.utils.networkalgorithms import (
    network_cycles_basis,network_cycles_simple,
    loop_edges_to_tree_index,
    associated_line_for_each_switch,
)

from ldrestoration_fast_test.utils.loggerconfig import setup_logging
...
def connectivity_vi_rule(model, i) -> Constraint:
    return (
            model.xij[i]
            <= model.vi[model.node_indices_in_tree[model.source_nodes[i]]]
    )
...
def connectivity_vj_rule(model, i) -> Constraint:
    return (
            model.xij[i]
            <= model.vi[model.node_indices_in_tree[model.target_nodes[i]]]
    )
...
class RestorationModel:
    ...
    def __initialize_model_data(self) -> None:
        ...
        ## added by shishir just to fix error, ignore it.
        # self.model.target_nodes = set(self.model.source_nodes).union(set(self.model.target_nodes))
        # self.model.target_nodes = tuple(self.model.target_nodes.difference({self.circuit_data["substation"]}))
    ...
    def __initialize_variables(self) -> None:
        ...
        self.model.si = Var(self.model.v_i, bounds=(0, 1), domain= binary_continuous) # added by shishir binary_continuous function
    ...
    def constraints_base(
        ...
        vsub_b: float = 1.05,
        vsub_c: float = 1.05,
        psub_a_max: float = 5000,
        psub_b_max: float = 5000,
        psub_c_max: float = 5000,
        DER_rating_constr_flag: bool = False,
        DER_p_max: float = 0.0,
    ) -> None:
        ...
        def voltage_balance_rule_base() -> None:
            ...
            for _, each_line in self.model.pdelements.iterrows():
                ...
                baseZ = each_line["base_kv_LL"] ** 2/3  # divided by 3 added by shishir later as single phase voltage need to be used, verify later
        ...
        def substation_positiveflow_rule_base() -> None:
            ...
            ## added by shishir (some logoical mistake was here, rather than substation index, we need line index emerging out from substation
            nodes_emerge_out_from_substation = list(self.network_graph.neighbors(self.circuit_data["substation"]))
            edge_index_list_out_from_substation = []
            for _ in  nodes_emerge_out_from_substation:
                try:
                    edge_index_list_out_from_substation.append(self.edge_indices_in_tree[(self.circuit_data["substation"], _)])
                except:
                    edge_index_list_out_from_substation.append(_, self.edge_indices_in_tree[(self.circuit_data["substation"])])

            for edge_index in edge_index_list_out_from_substation:
                self.model.substation_positive_flow.add(
                    self.model.Pija[edge_index] >= 0
                )
                self.model.substation_positive_flow.add(
                    self.model.Pijb[edge_index] >= 0
                )
                self.model.substation_positive_flow.add(
                    self.model.Pijc[edge_index] >= 0
                )

                # cpmmented and added by shishir
            # since for DERs Pa+Pb+Pc <DERS rating, apply similar principle here as well, Pa+Pb+Pc <= Psubmax
            # by shishir on 4/10/2025
            #     self.model.substation_positive_flow.add(
            #             self.model.Pija[edge_index] + self.model.Pijb[edge_index] + self.model.Pijc[edge_index] <= psub_max
            #         )
            ...
            # added by Shishir on 4/10/2025. If island is formed, ensure that power flowing from DER is within its capacity
                if DER_rating_constr_flag:
                    self.model.substation_positive_flow.add(
                        self.model.Pija[edge_index] + self.model.Pijb[edge_index] + self.model.Pijc[edge_index] <= DER_p_max)
            # self.model.substation_positive_flow.add(
            logger.info(
        ...
        def radiality_rule_base() -> None:
            ...
            for loops in self.model.loop_edge_idxs:
                ...
                if switches_in_loop: # added this conditional by shishir
        ...
        def der_limit_rule_base() -> None:
            ...
            for _, each_row in self.model.DERs.iterrows():
                ...
                # lower limit of DERs should be 0 but if positive substation power constraint is active, it can be ignored  # uncommented by Shishir
    ...
    def solve_model(
        ...
        tee:bool = True,  # this tee is added by shishir
        **kwargs,
    ) -> ConcreteModel | Any:
        ...
        if solver == "gurobi":
            ...
            for option in solver_options.keys(): # added by shishir
                optimization_solver.options[option] = solver_options[option]
        else:
            optimization_solver = SolverFactory(solver)

        results = optimization_solver.solve(self.model,tee = tee, **kwargs)
