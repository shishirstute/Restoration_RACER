...
temp_dir = current_working_dir + "/temp"  # temporary directcory for temporary purpose
...
for area_index in parent_child_area_dict.keys(): # iterating over each area
    ...
    if parent_area is not None: # if parent area is present , only update data. No need to do iterate over child as it will be automatically covered
        for _ in [parent_area]:
            ...
            dummy_bus_name = _ + area_index # dummpy bus name
            ...
            if to_bus not in bus_data["name"].to_list(): # to make sure that from bus is in parent side and to bus is in child side
